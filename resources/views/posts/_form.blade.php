...
<div class="form-group">
    ...
    <textarea class="form-control{{ $errors->has('preview') ? ' is-invalid' : '' }}" name="preview"
              rows="5">{{old('preview', $post->preview)}}</textarea>
...
<div class="form-group">
    ...
    <select class="js-example-basic-multiple form-control{{ $errors->has('tags') ? ' is-invalid' : '' }}"
            name="tags[]" multiple="multiple">
        @foreach($tags as $tag)
            <option value="{{$tag->id}}"
                    @if(in_array($tag->id, old('tags', $post->tags->pluck('id')->toArray()))) selected="" @endif>{{$tag->name}}</option>
        @endforeach
    </select>
